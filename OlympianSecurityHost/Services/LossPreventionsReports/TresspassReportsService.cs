...
{
    public class TresspassReportsService : ITresspassReportsService
    {

        private readonly HostDbContext db;
        private readonly IConfiguration Configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public TresspassReportsService(HostDbContext context, IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            db = context;
        ...
        public async Task<bool> Save(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.TresspassReports.Add(new TresspassReportsModel
                {
        ...
        public async Task<bool> Delete(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.TresspassReports.FirstOrDefault(x => x.SysSerial == model.SysSerial);
                if (obj != null)
                {
                    obj.IsDeleted = true;
        ...
        public async Task<LossPreventionsViewModel> Get(int id)
        {
            try
            {
                var obj = await db.TresspassReports.FirstOrDefaultAsync(x => x.SysSerial == id);
                if (obj != null)
                {
        ...
        public async Task<List<LossPreventionsViewModel>> GetAll()
        {
            try
            {
                ...
                List<TresspassReportsModel> obj;

                obj = await db.TresspassReports.Where(x => x.IsDeleted == false).ToListAsync();
