...
{
    public class YouthRightToCounselsService : IYouthRightToCounselsService
    {
        private readonly HostDbContext db;
        private readonly IConfiguration Configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public YouthRightToCounselsService(HostDbContext context, IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            db = context;
        ...
        public async Task<bool> Save(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.YouthRightToCounsels.Add(new YouthRightToCounselsModel
                {
        ...
        public async Task<bool> Delete(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.YouthRightToCounsels.FirstOrDefault(x => x.SysSerial == model.SysSerial);
                if (obj != null)
                {
                    obj.IsDeleted = true;
        ...
        public async Task<LossPreventionsViewModel> Get(int id)
        {
            try
            {
                var obj = await db.YouthRightToCounsels.FirstOrDefaultAsync(x => x.SysSerial == id);
                if (obj != null)
                {
        ...
        public async Task<List<LossPreventionsViewModel>> GetAll()
        {
            try
            {
                ...
                List<YouthRightToCounselsModel> obj;

                obj = await db.YouthRightToCounsels.Where(x => x.IsDeleted == false).ToListAsync();
