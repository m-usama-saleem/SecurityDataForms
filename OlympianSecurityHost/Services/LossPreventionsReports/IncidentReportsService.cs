...
namespace OlympianSecurityHost.Services.LossPreventionsReports
{
    public class IncidentReportsService : IIncidentReportsService
    {
        ...
        public IncidentReportsService(HostDbContext context, IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            db = context;
        ...
        public async Task<bool> Save(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.IncidentReports.Add(new IncidentReportsModel
                {
        ...
        public async Task<bool> Delete(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.IncidentReports.FirstOrDefault(x => x.SysSerial == model.SysSerial);
                if (obj != null)
                {
                    obj.IsDeleted = true;
        ...
        public async Task<LossPreventionsViewModel> Get(int id)
        {
            try
            {
                var obj = await db.IncidentReports.FirstOrDefaultAsync(x => x.SysSerial == id);
                if (obj != null)
                {
        ...
        public async Task<List<LossPreventionsViewModel>> GetAll()
        {
            try
            {
                ...
                List<IncidentReportsModel> obj;

                obj = await db.IncidentReports.Where(x => x.IsDeleted == false).ToListAsync();
