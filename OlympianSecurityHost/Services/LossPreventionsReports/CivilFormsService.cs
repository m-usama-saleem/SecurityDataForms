...
{
    public class CivilFormsService : ICivilFormsService
    {
        private readonly HostDbContext db;
        private readonly IConfiguration Configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public CivilFormsService(HostDbContext context, IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            db = context;
        ...
        public async Task<bool> Save(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.CivilForms.Add(new CivilFormsModel
                {
        ...
        public async Task<bool> Delete(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.CivilForms.FirstOrDefault(x => x.SysSerial == model.SysSerial);
                if (obj != null)
                {
                    obj.IsDeleted = true;
        ...
        public async Task<LossPreventionsViewModel> Get(int id)
        {
            try
            {
                var obj = await db.CivilForms.FirstOrDefaultAsync(x => x.SysSerial == id);
                if (obj != null)
                {
        ...
        public async Task<List<LossPreventionsViewModel>> GetAll()
        {
            try
            {
                ...
                List<CivilFormsModel> obj;

                obj = await db.CivilForms.Where(x => x.IsDeleted == false).ToListAsync();
