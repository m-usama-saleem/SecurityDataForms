...
{
    public class EmployeeTimeSheetsService : IEmployeeTimeSheetsService
    {
        private readonly HostDbContext db;
        private readonly IConfiguration Configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public EmployeeTimeSheetsService(HostDbContext context, IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            db = context;
        ...
        public async Task<bool> Save(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.EmployeeTimeSheets.Add(new EmployeeTimeSheetsModel
                {
        ...
        public async Task<bool> Delete(LossPreventionsViewModel model)
        {
            try
            {
                var obj = db.EmployeeTimeSheets.FirstOrDefault(x => x.SysSerial == model.SysSerial);
                if (obj != null)
                {
                    obj.IsDeleted = true;
        ...
        public async Task<LossPreventionsViewModel> Get(int id)
        {
            try
            {
                var obj = await db.EmployeeTimeSheets.FirstOrDefaultAsync(x => x.SysSerial == id);
                if (obj != null)
                {
        ...
        public async Task<List<LossPreventionsViewModel>> GetAll()
        {
            try
            {
                ...
                List<EmployeeTimeSheetsModel> obj;

                obj = await db.EmployeeTimeSheets.Where(x => x.IsDeleted == false).ToListAsync();
